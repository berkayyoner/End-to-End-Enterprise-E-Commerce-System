...
* Top navigation bar needs to have logo picture which leads to home page when it gets clicked, a search bar for products search which will work with elasticsearch, then "My Account", "My Favorites", "My Basket", light/dark mode and language changing buttons.
* Public applications home page and product pages are not required to log in. Users should be able to see the home page, should search products and see the details. But when they click "Buy Now", "Add to Basket", "Sell on [Project name]" without being logged in, it should direct the user the login page.
* When user puts mouse on the "Categories" button, it hovers directly without clicking. "Categories" menu shows main product types vertically. Automaticly hovers the sub types on right side of the main types lists inner types under the sub type titles. Only shows 5 inner types for for each title, 6th button is always "Show More" and leads to items page and filters for that items. So right order of category levels are: Main Categories > Sub Types > Inner Types. Main categories should be requested when the website loads but sub types and inner types should be requested when user hovers the "Categories" list and a main category from there. The first main category should open automatically when the menu hovers.
