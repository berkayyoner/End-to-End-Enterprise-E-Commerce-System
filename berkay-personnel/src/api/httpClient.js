...
/**
 * auth-service's personnel SecurityConfig chain issues a fresh CSRF cookie/token pair, and the
 * token gets rotated by Spring Security on every successful authentication (login) - so rather
 * than caching one, every mutating call (other than /login, which has no prior session) fetches
 * a fresh token right before it's used.
 */
async function fetchCsrfToken() {
  const response = await fetch(`${API_BASE_URL.replace('/personnel', '')}/csrf-token`, { credentials: 'include' })
  if (!response.ok) {
